  ...
  return (tree, file) => {
    visit(tree, "element", (node) => {
      ...
      */
      ...
      const resolvedUrl = [
        collectionName === NO_COLLECTION_NAME
          ? ""
          : path.posix.sep + collectionName,
        resolvedSlug,
      ].join(path.posix.sep);
      ...
      let webPathFinal = applyTrailingSlash(
        (frontmatterSlug === "" ? "/" : frontmatterSlug) || url,
        resolvedUrl,
        trailingSlashMode,
      );
      ...
      debug(
        "File relative to collection path   : %s",
